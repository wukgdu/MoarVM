...
** DONE Take pointers out of the expression tree
...
** Take label assignment to tiling

Currently the labels are assigned (relative to a undefined tree
internal label offset) during tree constructing (in the preorder phase
of =MVM_jit_expr_tree_analyze=). I want to move this to the tiling
phase because:

- this is the only place where we actually need them
- also, we know the absolute label offset, so there is no reason to
  have a relative label offset anymore
- during tiling, we break it in the optimization step anymore
- we can keep the node-to-label map as a temporary data structure
  rather than directly ('permanently') associated with the tree
...
typedef struct {
    MVMint16 o;     /* operator */
    MVMuint8 n;     /* number of children */
    MVMuint8 s : 4; /* result size */
    MVMuint8 t : 4; /* result type */
} MVMJitExprNode;

typedef struct {
    union {
        struct { MVM_VECTOR_DECL(MVMint32, nodes); };
        struct { MVM_VECTOR_DECL(MVMJitExprNode, info); };
    };
    ...
} MVMJItExprTree;
#+END_SRC

Because we can then still access the nodes as integer numbers
directly, and none of that code actually breaks.
...
** Implement =COND= and associated operators

The idea is that we can generalize all conditional operators like
=WHEN= and =IF= and =IFV= as =COND= (like LISP) operators. The goal of
doing so is that in theory, compiling =COND= is more regular than
compiling variants of =IF= and =IFV=.

Needs some care in the case of yielding-a-value (and we might still
implement an =^if= macro) for the linear scan allocator.
